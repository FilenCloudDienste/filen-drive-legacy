...
const List = memo(
	({
		...
		const [listScrollState, setListScrollState] = useState<ListScrollState>({
			clientHeight: windowHeight,
			scrollHeight: LIST_ITEM_HEIGHT * items.length,
			scrollTop: 0
		})
		...
		useEffect(() => {
			setListScrollState({
				clientHeight: windowHeight,
				scrollHeight: (gridFolders[window.location.href] ? GRID_CELL_HEIGHT : LIST_ITEM_HEIGHT) * items.length,
				scrollTop: 0
			})
