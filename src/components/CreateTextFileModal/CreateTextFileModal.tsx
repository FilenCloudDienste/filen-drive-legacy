...
export const CreateTextFileModalEditor = memo(
	({ darkMode, isMobile, lang, windowHeight, windowWidth }: CreateFolderModalProps) => {
		const [open, setOpen] = useState<boolean>(false)
		const [newName, setNewName] = useState<string>("")

		useEffect(() => {
			const openCreateTextFileModalEditorListener = eventListener.on(
				"openCreateTextFileModalEditor",
				(n: string) => {
			...
			const closeCreateTextFileModalEditorListener = eventListener.on(
				"closeCreateTextFileModalEditor",
				(n: string) => {
					memoryCache.set("textEditorChanged", false)
...
export const CreateTextFileModal = memo(
	({ darkMode, isMobile, windowHeight, windowWidth, setItems, items, lang }: CreateFolderModalProps) => {
		...
		const create = useCallback(() => {
			...
			if (
				currentItems.current.filter(
					item => item.name.toLowerCase() == value.toLowerCase() && item.type == "file"
				).length > 0
			) {
				showToast("error", i18n(lang, "pleaseChooseDiffName"), "bottom", 5000)
