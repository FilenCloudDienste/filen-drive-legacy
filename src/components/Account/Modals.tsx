	...
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		...
		const save = useCallback(async () => {
			...
			try {
				...
				const { derivedPassword } = await generatePasswordAndMasterKeysBasedOnAuthVersion(
					password.trim(),
					authVersion,
					salt
				)
...
export const PersonalModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
		...
		const fetchAccount = useCallback(async () => {
			...
			try {
				...
				setPersonal({
					city:
						typeof account.personal.city == "string" && account.personal.city.length > 0
							? account.personal.city
							: "",
					companyName:
						typeof account.personal.companyName == "string" && account.personal.companyName.length > 0
							...
							: "",
					country:
						typeof account.personal.country == "string" && account.personal.country.length > 0
					...
					firstName:
						typeof account.personal.firstName == "string" && account.personal.firstName.length > 0
							...
							: "",
					lastName:
						typeof account.personal.lastName == "string" && account.personal.lastName.length > 0
					...
					postalCode:
						typeof account.personal.postalCode == "string" && account.personal.postalCode.length > 0
							...
							: "",
					street:
						typeof account.personal.street == "string" && account.personal.street.length > 0
							? account.personal.street
							: "",
					streetNumber:
						typeof account.personal.streetNumber == "string" && account.personal.streetNumber.length > 0
							...
							: "",
					vatId:
						typeof account.personal.vatId == "string" && account.personal.vatId.length > 0
							? account.personal.vatId
							: ""
				})
			} catch (e: any) {
				console.error(e)
									...
									>
										...
										>
											...
											<Input
												...
												onChange={e =>
													setPersonal(prev => ({ ...prev, companyName: e.target.value }))
												}
										...
										>
											...
											<Input
												...
												onChange={e =>
													setPersonal(prev => ({ ...prev, vatId: e.target.value }))
												}
									...
									>
										...
										>
											...
											<Input
												...
												onChange={e =>
													setPersonal(prev => ({ ...prev, street: e.target.value }))
												}
										...
										>
											...
											<Input
												...
												onChange={e =>
													setPersonal(prev => ({ ...prev, streetNumber: e.target.value }))
												}
									...
									>
										...
										>
											...
											<Input
												...
												onChange={e =>
													setPersonal(prev => ({ ...prev, postalCode: e.target.value }))
												}
...
export const DeleteVersionedModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
...
export const DeleteAllModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
	...
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		...
		const save = useCallback(async () => {
			...
			try {
				...
				const { derivedPassword: newPasswordHash, derivedMasterKeys } =
					await generatePasswordAndMasterKeysBasedOnAuthVersion(newPassword.trim(), authVersion, newSalt)
...
export const TwoFactorModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
...
export const TwoFactorRecoveryInfoModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
...
export const DisableTwoFactorModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
		const [code, setCode] = useState<string>("")
...
export const ExportMasterKeysModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
							...
							<Flex
								...
								onClick={() =>
									copy(
										Base64.encode(
											masterKeys
												.map(key => "_VALID_FILEN_MASTERKEY_" + key + "_VALID_FILEN_MASTERKEY_")
												.join("|")
										)
									)
								}
...
export const AffiliatePayoutModal = memo(
	({ darkMode, isMobile, lang }: { darkMode: boolean; isMobile: boolean; lang: string }) => {
		const [open, setOpen] = useState<boolean>(false)
		const [loading, setLoading] = useState<boolean>(false)
