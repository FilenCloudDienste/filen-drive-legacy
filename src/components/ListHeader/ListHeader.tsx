...
const ListHeader = memo(({ darkMode, isMobile, items, setItems, loadingItems, listWidth, lang }: ListHeaderProps) => {
    ...
    const [markerWidth, nameWidth, sizeWidth, lastModifiedWidth, actionsWidth] = useMemo(() => {
        ...
        const nameWidth: number = Math.floor(listWidthMinusMarker * (isMobile ? 0.50 : 0.70)) - (isMobile ? 8 : 13)
        const sizeWidth: number = Math.floor(listWidthMinusMarker * (isMobile ? 0.20 : 0.10))
        const lastModifiedWidth: number = Math.floor(listWidthMinusMarker * (isMobile ? 0.30 : 0.20))
        ...
        >
            <Flex
                ...
                onClick={() => {
                    setSortBy({
                        ...sortBy,
                        [window.location.href]: typeof sortBy[window.location.href] == "undefined" || sortBy[window.location.href] == "nameAsc" ? "nameDesc" : "nameAsc"
                    })
                }}
            >
                <AppText
            ...
            <Flex
                ...
                onClick={() => {
                    setSortBy({
                        ...sortBy,
                        [window.location.href]: typeof sortBy[window.location.href] == "undefined" || sortBy[window.location.href] == "sizeAsc" ? "sizeDesc" : "sizeAsc"
                    })
                }}
            >
                <AppText
            ...
            <Flex
                ...
                onClick={() => {
                    setSortBy({
                        ...sortBy,
                        [window.location.href]: typeof sortBy[window.location.href] == "undefined" || sortBy[window.location.href] == "lastModifiedAsc" ? "lastModifiedDesc" : "lastModifiedAsc"
                    })
                }}
            >
                <AppText
