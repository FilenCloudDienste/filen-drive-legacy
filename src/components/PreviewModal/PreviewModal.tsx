...
const PreviewModal = memo(
	({ darkMode, isMobile, windowHeight, windowWidth, setItems, items, lang }: PreviewModalProps) => {
		...
		const loadPreview = useCallback((item: ItemProps) => {
			...
			setItems(prev =>
				prev.map(mapItem =>
					mapItem.uuid == item.uuid ? { ...mapItem, seleted: true } : { ...mapItem, seleted: false }
				)
			)
			setType(previewType)
			setCurrentItem(item)
		...
		useEffect(() => {
			const openPreviewModalListener = eventListener.on(
				"openPreviewModal",
				({ item, items: passedItems }: { item: ItemProps; items: ItemProps[] }) => {
					loadPreview(item)
				}
			)
