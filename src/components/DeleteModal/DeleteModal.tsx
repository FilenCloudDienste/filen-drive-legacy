...
const DeleteModal = memo(
	({ darkMode, isMobile, windowHeight, windowWidth, setItems, items, lang }: DeleteModalProps) => {
		...
		const deleteItems = useCallback(async () => {
			...
			) as PromiseFulfilledResult<ItemProps>[]
			...
			if (deleted.length > 0) {
				showToast(
					"success",
					i18n(lang, "itemsMovedToTrash", true, ["__COUNT__"], [success.length.toString()]),
					"bottom",
					5000
				)
