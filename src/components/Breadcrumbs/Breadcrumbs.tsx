        ...
        try{
            ...
            for(let i = 0; i < validFolders.length; i++){
                if(validFolders[i] == defaultDriveUUID){
                    ...
                    crumbs.push({
                        uuid: validFolders[i],
                        name: typeof folderNames[validFolders[i]] == "string" ? folderNames[validFolders[i]] : memoryCache.has("folderName:" + validFolders[i]) ? memoryCache.get("folderName:" + validFolders[i]) : validFolders[i]
                    })
                }
            }
