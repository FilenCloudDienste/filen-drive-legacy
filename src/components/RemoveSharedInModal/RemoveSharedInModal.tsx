    ...
    const removeSharedIn = useCallback(async () => {
        ...
        for(let i = 0; i < removeSharedInItems.current.length; i++){
            promises.push(new Promise((resolve, reject) => {
                removeSharedInItem(removeSharedInItems.current[i]).then(() => {
        ...
        if(error.length > 0){
            for(let i = 0; i < error.length; i++){
                showToast("error", i18n(lang, "couldNotRemoveSharedItems", true, ["__NAME__", "__ERR__"], [error[i].reason.item.name, error[i].reason.err.toString()]), "bottom", 5000)
            }
        }
        ...
        if(removed.length > 0){
            ...
            showToast("success", i18n(lang, "removedSharedItems", true, ["__COUNT__"], [removeSharedInItems.current.length.toString()]), "bottom", 5000)
