...
export const CloudTreeItem = memo(({ darkMode, isMobile, parent, depth, folders, loading, path, moveModalFolderOpen, setMoveModalFolderOpen, selected, setSelected, toMove }: { darkMode: boolean, isMobile: boolean, parent: ItemProps, depth: number, folders: ItemProps[], loading: boolean, path: string, moveModalFolderOpen: { [key: string]: boolean }, setMoveModalFolderOpen: React.Dispatch<React.SetStateAction<{ [key: string]: boolean }>>, selected: ItemProps | undefined, setSelected: React.Dispatch<React.SetStateAction<ItemProps | undefined>>, toMove: ItemProps[] }) => {
    ...
    return (
        ...
        >
            ...
            >
                {
                    moveModalFolderOpen[parent.uuid] && folders.length > 0 && folders.map((folder) => {
                        return (
                            <CloudTree
                                key={folder.uuid}
                                darkMode={darkMode}
...
export const CloudTree = memo(({ darkMode, isMobile, depth, parent, path, moveModalFolderOpen, setMoveModalFolderOpen, selected, setSelected, toMove }: { darkMode: boolean, isMobile: boolean, depth: number, parent: ItemProps, path: string, moveModalFolderOpen: { [key: string]: boolean }, setMoveModalFolderOpen: React.Dispatch<React.SetStateAction<{ [key: string]: boolean }>>, selected: ItemProps | undefined, setSelected: React.Dispatch<React.SetStateAction<ItemProps | undefined>>, toMove: ItemProps[] }) => {
                ...
                <ModalFooter>
                    {
                        loading || typeof selected == "undefined" ? (
                            ...
                            <AppText
                                ...
                                onClick={() => move()}
                            >
                                {i18n(lang, "moveModalBtn", true, ["__COUNT__", "__DEST__"], [toMove.length.toString(), (selected.name.length >= 16 ? (selected.name.length > 16 ? selected.name.slice(0, 16) + ".." : selected.name.slice(0, 16)) : selected.name)])}
                            </AppText>
                        )
                    }
                </ModalFooter>
