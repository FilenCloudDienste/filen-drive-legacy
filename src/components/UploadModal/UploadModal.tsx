...
import type { UploadModalProps, UploadQueueItem, UploadQueueItemFile, UploadModalListProps, UploadModalListItemProps, CurrentUpload, ItemProps } from "../../types"
import { List as RVList, AutoSizer } from "react-virtualized"
import AppText from "../AppText"
import { v4 as uuidv4 } from "uuid"
import { queueFileUpload } from "../../lib/services/upload"
import { getCurrentURLParentFolder, calcSpeed, calcTimeLeft, getTimeRemaining, bpsToReadable, pathGetDirname, pathGetBasename, getEveryPossibleFolderPathFromPath } from "../../lib/helpers"
...
const UploadModal = memo(({ darkMode, isMobile, windowWidth, windowHeight, lang, items }: UploadModalProps) => {
    ...
    useEffect(() => {
        const openUploadModalListener = eventListener.on("openUploadModal", async ({ files = undefined, openModal = true }: { files?: UploadQueueItemFile[], openModal?: boolean }) => {
            if(Array.isArray(files)){
                ...
                if(needsToCreateFolders){
                    ...
                    folderPathsToCreate = [...folderPathsToCreate, ...possible].sort((a, b) => a.split("/").length - b.split("/").length)
                    ...
                    updateToast(createFolderToast, "loading", i18n(lang, "creatingFoldersProgress", true, ["__LEFT__"], [folderPathsToCreate.length.toString()]), ONE_YEAR)
                    ...
                    for(let i = 0; i < folderPathsToCreate.length; i++){
                        ...
                        updateToast(createFolderToast, "loading", i18n(lang, "creatingFoldersProgress", true, ["__LEFT__"], [(folderPathsToCreate.length - i).toString()]), ONE_YEAR)
                    }
                ...
                >
                    {
                        (Object.keys(currentUploads).length + Object.keys(finishedTransfers).length) > 0 ? (
                            <UploadModalList
                                ...
                                isMobile={isMobile}
                                lang={lang}
                                currentUploads={[...Object.keys(currentUploads).map(key => currentUploads[key]).sort((a, b) => (isNaN(b.percent) ? 0 : b.percent) - (isNaN(a.percent) ? 0 : a.percent)), ...Object.keys(erroredTransfers).map(key => erroredTransfers[key]).sort((a, b) => b.timestamp - a.timestamp), ...Object.keys(finishedTransfers).map(key => finishedTransfers[key]).sort((a, b) => b.timestamp - a.timestamp)]}
                            />
                        ) : (
                            <Flex
                                height="100%"
                        ...
                        >
                            {
                                Object.keys(currentUploads).length > 0 && (() => {
                                    ...
                                    return i18n(lang, "aboutRemaining", false, ["__TIME__"], [(remainingReadable.days > 0 ? remainingReadable.days + "d " : "") + (remainingReadable.hours > 0 ? remainingReadable.hours + "h " : "") + (remainingReadable.minutes > 0 ? remainingReadable.minutes + "m " : "") + (remainingReadable.seconds > 0 ? remainingReadable.seconds + "s " : "")]) + ", " + bpsToReadable(speed)
                                })()
                            }
                        </AppText>
                    </Flex>
