...
export const apiRequest = async ({ method = "POST", endpoint, data }: { method: string, endpoint: string, data: any }): Promise<any> => {
    const response = await getWorkerAPI().apiRequest(method, endpoint, data)

    if(typeof response == "object"){
        if(typeof response.message == "string"){
            if(response.message.toLowerCase().indexOf("api key not found") !== -1 || response.message.toLowerCase().indexOf("invalid api key") !== -1){
                logout()
...
export const generatePasswordAndMasterKeysBasedOnAuthVersion = async (rawPassword: string, authVersion: number, salt: string): Promise<{ derivedMasterKeys: string, derivedPassword: string }> => {
    return await getWorkerAPI().generatePasswordAndMasterKeysBasedOnAuthVersion(rawPassword, authVersion, salt)
}

export const deriveKeyFromPassword = async (password: string, salt: string, iterations: number, hash: string, bitLength: number, returnHex: boolean): Promise<string | ArrayBuffer> => {
	return await getWorkerAPI().deriveKeyFromPassword(password, salt, iterations, hash, bitLength, returnHex)
}

export const hashPassword = async (password: string,): Promise<string> => { //old and deprecated, no longer in use
...
export const decryptFileMetadata = async (metadata: string, masterKeys: string[]): Promise<{ name: string, size: number, mime: string, key: string, lastModified: number }> => {
