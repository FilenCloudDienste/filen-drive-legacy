...
const useTransfers = ({ onUploadStart, onUploadStarted, onUploadDone, onUploadError, onDownloadStart, onDownloadStarted, onDownloadDone, onDownloadError, onUploadProgress, onDownloadProgress, onTransferStopped }: UseTransfersParams): UseTransfers => {
    const [currentUploads, setCurrentUploads] = useState<{ [key: string]: CurrentUpload }>({})
    const [currentDownloads, setCurrentDownloads] = useState<{ [key: string]: CurrentDownload }>({})
    ...
    useEffect(() => {
        const uploadListener = eventListener.on("upload", (data: Upload) => {
            ...
            if(data.type == "start"){
                ...
                setCurrentUploads(prev => Object.keys(prev).filter(key => key !== data.data.uuid).reduce((current, key) => Object.assign(current, { [key]: prev[key] }), {}))
            }
        ...
        const downloadListener = eventListener.on("download", (data: Download) => {
            ...
            if(data.type == "start"){
                ...
                setCurrentDownloads(prev => Object.keys(prev).filter(key => key !== data.data.uuid).reduce((current, key) => Object.assign(current, { [key]: prev[key] }), {}))
            }
            else if(data.type == "err"){
        ...
        const uploadProgressListener = eventListener.on("uploadProgress", (data: ProgressData) => {
            ...
            setCurrentUploads(prev => Object.keys(prev).filter(key => key == data.data.uuid).length > 0 ? ({
                ...prev,
                [data.data.uuid]: {
                    ...prev[data.data.uuid],
                    percent: ((prev[data.data.uuid].bytes + data.data.bytes) / Math.floor((prev[data.data.uuid].file.size || 0) * 1)) * 100,
                    ...
                    timeLeft: calcTimeLeft((prev[data.data.uuid].bytes + data.data.bytes), Math.floor((prev[data.data.uuid].file.size || 0) * 1), prev[data.data.uuid].started)
                }
            }) : prev)
        })
        ...
        const downloadProgressListener = eventListener.on("downloadProgress", (data: ProgressData) => {
            ...
            setCurrentDownloads(prev => Object.keys(prev).filter(key => key == data.data.uuid).length > 0 ? ({
                ...prev,
                [data.data.uuid]: {
                    ...prev[data.data.uuid],
                    percent: ((prev[data.data.uuid].bytes + data.data.bytes) / Math.floor((prev[data.data.uuid].file.size || 0) * 1)) * 100,
        ...
        const stopTransferListener = eventListener.on("stopTransfer", (uuid: string) => {
            ...
            setCurrentUploads(prev => Object.keys(prev).filter(key => key !== uuid).reduce((current, key) => Object.assign(current, { [key]: prev[key] }), {}))
            setCurrentDownloads(prev => Object.keys(prev).filter(key => key !== uuid).reduce((current, key) => Object.assign(current, { [key]: prev[key] }), {}))
        })
