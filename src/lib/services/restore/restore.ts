...
export const restoreFromTrash = async (items: ItemProps[]): Promise<void> => {
    ...
    if(error.length > 0){
        for(let i = 0; i < error.length; i++){
            showToast("error", i18n(lang, "couldNotRestoreItem", true, ["__NAME__", "__ERR__"], [error[i].reason.item.name, error[i].reason.err.toString()]), "bottom", 5000)
        }
    }
