...
export const getEventText = async ({ item, masterKeys, lang }: GetEventText) => {
	...
	switch (item.type) {
		...
		case "fileRenamed":
			...
			eventText = i18n(
				lang,
				"eventFileRenamedInfo",
				true,
				["__NAME__", "__NEW__"],
				[striptags(decryptedOld.name), striptags(decrypted.name)]
			)
			break
		case "fileTrash":
			decrypted = await decryptFileMetadata(item.info.metadata, masterKeys)
		...
		case "fileShared":
			...
			eventText = i18n(
				...
				true,
				["__NAME__", "__EMAIL__"],
				[striptags(decrypted.name), item.info.receiverEmail]
			)
			break
		case "fileLinkEdited":
			decrypted = decrypted = await decryptFileMetadata(item.info.metadata, masterKeys)
		...
		case "folderShared":
			...
			eventText = i18n(
				lang,
				"eventFolderSharedInfo",
				true,
				["__NAME__", "__EMAIL__"],
				[striptags(decrypted), item.info.receiverEmail]
			)
			break
		case "folderMoved":
			decrypted = await decryptFolderName(item.info.name, masterKeys)
		...
		case "removedSharedInItems":
			eventText = i18n(
				lang,
				"eventRemovedSharedInItemsInfo",
				true,
				["__COUNT__", "__EMAIL__"],
				[item.info.count, item.info.sharerEmail]
			)
			break
		case "removedSharedOutItems":
			eventText = i18n(
				lang,
				"eventRemovedSharedOutItemsInfo",
				true,
				["__COUNT__", "__EMAIL__"],
				[item.info.count, item.info.receiverEmail]
			)
			break
		default:
			eventText = item.type
