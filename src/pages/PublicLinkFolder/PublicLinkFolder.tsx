...
const PublicLinkFolder = memo(({ windowWidth, windowHeight, darkMode, isMobile, lang }: AppBaseProps) => {
    ...
    const fetchInfo = useCallback(() => {
        if(typeof params.uuid == "string" && typeof key == "string"){
            if(validateUUID(params.uuid) && key.length == 32){
                folderLinkInfo(params.uuid as string).then(async (linkInfo) => {
                    setInfo(linkInfo)
    ...
    const downloadFolder = useCallback(async (loadCallback: any) => {
        ...
        if(url.length > 0){
            ...
            if(validateUUID(parent) && typeof params.uuid == "string" && validateUUID(params.uuid) && typeof info !== "undefined"){
                const getContents = async () => {
                    ...
                    folderLinkContents(params.uuid as string, parent, info.hasPassword ? (info.salt.length == 32 ? (await deriveKeyFromPassword(password, info.salt, 200000, "SHA-512", 512, true) as string) : (await hashFn(password.length == 0 ? "empty" : password))) : (await hashFn("empty"))).then(async (content) => {
