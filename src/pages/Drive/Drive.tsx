...
import type { ItemProps, ItemDragState, DragSelectState, ItemFavoritedEvent, ItemMovedEvent, ItemRestoredEvent, FileUploadedEvent, AddFolderEvent, ItemColorChangedEvent, ItemUpdatedEvent } from "../../types"
import { getDragSelectCoords, getDragSelectCollisions, orderItemsByType, getCurrentURLParentFolder, readLocalDroppedDirectory, getCurrentParent, convertTimestampToMs } from "../../lib/helpers"
...
const Drive = memo(({ windowWidth, windowHeight, darkMode, isMobile, lang }: AppBaseProps) => {
    ...
    const [dragSelectState, setDragSelectState] = useState<DragSelectState>({ start: { clientX: 0, clientY: 0 }, current: { clientX: 0, clientY: 0 } })
    ...
    const mouseDownListener = useCallback(throttle((e: MouseEvent): void => {
        if(memoryCache.has("previewModalOpen") || window.location.hash.indexOf("account") !== -1 || document.querySelectorAll(".chakra-modal__overlay").length > 0){
            return
        }

        if(isDragSelecting.current || e.button !== 0 || dragSelectStart.current.clientX !== 0 || dragSelectStart.current.clientY !== 0){
            return
        }
    ...
    const windowOnKeyDown = useCallback((e: KeyboardEvent): void => {
        ...
        if(e.ctrlKey && e.which == 65 && currentSearchTerm.current.length == 0 && document.querySelectorAll(".chakra-modal__overlay").length == 0){
    ...
    const bodyOnClickListener = useCallback((e: MouseEvent): void => {
        ...
        if(path.filter(el => typeof el !== "undefined" && typeof el.className == "string" && el.className.indexOf("do-not-unselect-items") !== -1).length == 0 && !isDragSelecting.current){
    ...
    const populateList = useCallback(async (refresh: boolean = false): Promise<void> => {
        ...
        loadItems(startingURL, refresh).then((loadedItems) => {
        ...
        if(initDone){
            if(location.hash.split("/").length < 2){
                db.get("defaultDriveUUID").then((defaultDriveUUID) => {
                    if(typeof defaultDriveUUID == "string"){
                        if(defaultDriveUUID.length >= 32){
            ...
            if(event.type == "fileArchived" || event.type == "fileTrash" || event.type == "folderTrash"){
                ...
                if(typeof decrypted.name == "string"){
                    if(decrypted.name.length > 0){
                        setItems(prev => prev.map(item => item.uuid == event.data.uuid ? { ...item, name: striptags(decrypted.name) } : item))
                    }
                }
            }
                ...
                if(getCurrentParent() == event.data.parent){
                    ...
                    if(typeof decrypted == "string"){
                        if(decrypted.length > 0){
                            setItems(prev => {
                                if(getCurrentParent() == event.data.parent && prev.filter(item => item.uuid == event.data.uuid).length == 0){
                        ...
                        if(decrypted.name.length > 0){
                            setItems(prev => {
                                if(getCurrentParent() == event.data.parent && prev.filter(item => item.uuid == event.data.uuid).length == 0){
                                    ...
                                    setItems(prev => orderItemsByType([...prev, ...[{ ...newItem, selected: false }]], sortBy[window.location.href], window.location.href))
