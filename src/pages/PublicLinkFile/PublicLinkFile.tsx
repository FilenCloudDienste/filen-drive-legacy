...
const PublicLinkFile = memo(({ windowWidth, windowHeight, darkMode, isMobile, lang }: AppBaseProps) => {
    ...
    const download = useCallback(() => {
        ...
        queueFileDownload(getItemFromFile(info as LinkGetInfoV1, file as { name: string, size: number, mime: string }, key)).then(console.log).catch(console.error)
        ...
        if(typeof params.uuid == "string" && typeof key == "string"){
            if(validateUUID(params.uuid) && key.length == 32){
                const getInfo = async (passwordInfo: LinkHasPasswordV1) => {
                    ...
                    publicLinkInfo(params.uuid as string, passwordInfo.hasPassword ? (passwordInfo.salt.length == 32 ? (await deriveKeyFromPassword(password, passwordInfo.salt, 200000, "SHA-512", 512, true) as string) : (await hashFn(password.length == 0 ? "empty" : password))) : (await hashFn("empty"))).then(async (linkInfo) => {
                ...
                publicLinkHasPassword(params.uuid).then((passwordInfo) => {
